...
namespace rz
{
  ...
  template <typename T>
  concept JsonDeserializable = std::default_initializable<T> && requires(nlohmann::json j, T t) {
    j.get_to(t);
  };

  template <JsonDeserializable T>
  EXPECTED_UPTR<T> LoadObjFromJsonFile_UPTR(const std::filesystem::path& _filePath, const logCB& _logger = emptyCB){
  ...
  template <JsonDeserializable T>
  EXPECTED_OBJ<T> LoadObjectFromJsonFile_OBJ(const std::filesystem::path& _filePath, const logCB& _logger = emptyCB)
  {
    if(!std::filesystem::exists(_filePath) || !std::filesystem::is_regular_file(_filePath)){
      _logger(std::format("{} doesn't exist or isn't a regular file.\n", _filePath.string()));
  ...
  template <JsonDeserializable T>
  STATUS LoadObjectFromJsonFile_INPLACE(const std::filesystem::path& _filePath, T& _t, const logCB& _logger = emptyCB)
  {
    if(!std::filesystem::exists(_filePath) || !std::filesystem::is_regular_file(_filePath)){
      _logger(std::format("{} doesn't exist or isn't a regular file.\n", _filePath.string()));
      return STATUS::ERROR;
  ...
  STATUS WriteObjToJsonFile(const T& _t, const std::filesystem::path& _filePath, int _indent = 4, const logCB& _logger = emptyCB)
  {
    if(std::filesystem::exists(_filePath) && !std::filesystem::is_regular_file(_filePath)){
